...
export async function pullFromSupabase(storeName, tableName) {
  try {
    ...
    for (const item of remoteData) {
      // Converter de snake_case para camelCase antes de salvar no IndexedDB
      const itemCamelCase = toCamelCase(item);
      await idb.upsertItem(storeName, itemCamelCase);
    }
  ...
  try {
    ...
    for (const op of pendingOps) {
      try {
        ...
        console.log(`🔄 Processando operação ${op.id}:`, {
          store,
          tableName,
          operation,
          recordId,
        ...
        switch (operation) {
          case "create":
          case "update":
            // Converter campos para snake_case antes de enviar ao Supabase
            const dataToSync = toSnakeCase(data);
            console.log(`📤 Dados convertidos para snake_case:`, dataToSync);
            
            try {
              const result = await supabase.upsertItem(tableName, dataToSync);
        ...
        errors.push({ opId: op.id, error: error.message || String(error) });
      }
    }
...
export async function queueOperation(storeName, operation, data, recordId = null) {
  ...
  console.log("✅ Operação adicionada à fila com sucesso");
}
...
/**
 * Helper: Converte campos de camelCase para snake_case
 */
...
/**
 * Helper: Converte campos de snake_case para camelCase
 */
